...
class Discriminator(nn.Module):
    ...
    def ecal_angle(self, image, daxis1):
        ...
        x_ref = torch.sum(torch.sum(image, dim=(2, 3)) 
            * (torch.arange(x_shape, dtype=torch.float32).unsqueeze(0) + 0.5), 
            dim=1,) # sum for x position * x index
        y_ref = torch.sum(
            torch.sum(image, dim=(1, 3))
            * (torch.arange(y_shape, dtype=torch.float32).unsqueeze(0) + 0.5),
            dim=1,)
        z_ref = torch.sum(
            torch.sum(image, dim=(1, 2))
            * (torch.arange(z_shape, dtype=torch.float32).unsqueeze(0) + 0.5),
            dim=1,)
        ...
        x = torch.arange(x_shape).unsqueeze(0)  # x indexes
        x = (x.unsqueeze(2).float()) + 0.5
        y = torch.arange(y_shape).unsqueeze(0)  # y indexes
        ...
        # Angle Calculations
        z = (torch.arange(z_shape, dtype=torch.float32) + 0.5) * torch.ones_like(z_ref) # Make an array of z indexes for all events
